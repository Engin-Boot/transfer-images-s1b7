...
TEST_CASE("when the array of -b and port number and its corresponding position and storage option is given then setLocalPortNumber sets"
...
TEST_CASE("when the array of -n and host name and its corresponding position and storage option is given then setRemoteHostName sets"
...
TEST_CASE("when the  array of -p and its port number and its corresponding position and storage option is given then setRemotePortNumber sets"
...
TEST_CASE("when the command line argument and the argument count is given then the TestCmdLine function returns SAMP_TRUE indicating that all parameters are set")
{
...
TEST_CASE("When the node head is given then GetNumNodes returns the number of nodes")
{
	auto node1 = static_cast<InstanceNode*>(malloc(sizeof(InstanceNode)));
	auto node2 = static_cast<InstanceNode*>(malloc(sizeof(InstanceNode)));
...
TEST_CASE("When empty list node head is given then GetNumNodes returns 0 ")
{
	auto node1 = static_cast<InstanceNode*>(malloc(sizeof(InstanceNode)));
	auto node2 = static_cast<InstanceNode*>(malloc(sizeof(InstanceNode)));
...
TEST_CASE("When the list head is given then FreeList frees the linked list")
{
	auto node1 = static_cast<InstanceNode*>(malloc(sizeof(InstanceNode)));
	auto node2 = static_cast<InstanceNode*>(malloc(sizeof(InstanceNode)));
	auto node3 = static_cast<InstanceNode*>(malloc(sizeof(InstanceNode)));
...
TEST_CASE("When the instanceList pointer address,storage options with two images  and imageList is given then the addImageTOList function"
"adds the two images to the imagelist")
{
	STORAGE_OPTIONS                   options;
...
TEST_CASE("when the address of  applicationId ,AssociationId and storage Options is given with no library initialization then the"
	"openAssociation Function does not open the association")
{
...
TEST_CASE("when the storage options,applicationID and instanceList with no library initialization is given"
	"then read function return SAMP_FALSE")
{
...
TEST_CASE("when the storage options,associationID,and node is given  then send fucntion return SAMP_TRUE")
{
	SAMP_BOOLEAN                  sampBool;
	STORAGE_OPTIONS               options;
	int                           associationID = -1;
...
TEST_CASE("When the Storage options,associationId and instaceList is given with no successfull libraray initialization then"
